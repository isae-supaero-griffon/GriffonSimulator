...
def Simulation_debit_constant_fuel_sliver(ox_flow, geom, nozzle, safety_thickness, dt, show_curves):
    ...
    Interpreter = JsonInterpreter("Thermodynamic Data.json")
    ...
    while(geom.min_bloc_thickness() > safety_thickness):
        ...
        print("")
        fuel_flow = geom.compute_fuel_rate(rho_fuel, ox_flow, a, n, m)
        ...
        print("time : " + str(k*dt) + " s")
        print("min block thickness : " + str(1000 * geom.min_bloc_thickness()) + " mm")
    ...
    for k in range(len(time)):
        ...
        avg_thrust += thrust_curve[k]
        avg_Isp += isp_curve[k]
        avg_OF += OF_curve[k]
        total_impulse += dt * thrust_curve[k]

    avg_thrust = avg_thrust / len(thrust_curve)
